...
function malformedReasons(raw: string): string[] {
  ...
  if (raw !== raw.trim()) reasons.push("The secret has leading or trailing whitespace; paste only the API key value.");
  if (raw.includes("\n") || raw.includes("\r")) reasons.push("The secret contains newline characters; paste it as a single line.");
  if (/^OPENAI_API_KEY\s*=/.test(raw)) reasons.push("The secret includes the 'OPENAI_API_KEY=' prefix; save only the key value.");
  if (/^Bearer\s+/i.test(raw)) reasons.push("The secret includes a 'Bearer ' prefix; save only the key value.");
  if ((raw.startsWith('"') && raw.endsWith('"')) || (raw.startsWith("'") && raw.endsWith("'"))) reasons.push("The secret is wrapped in quotes; remove the quotes.");
  if (/%0A|%0D|%22|%27|%20/i.test(raw)) reasons.push("The secret appears URL-encoded; paste the raw OpenAI key instead.");
  if (!raw.startsWith("sk-")) reasons.push("The secret must start with 'sk-'.");
  if (/\s/.test(raw)) reasons.push("The secret contains whitespace characters.");
  if (!/^sk-[A-Za-z0-9_-]+$/.test(raw)) reasons.push("The secret contains characters outside the expected OpenAI key character set.");
  if (raw.length < 40) reasons.push("The secret is shorter than expected for an OpenAI API key.");
...
export function inspectOpenAiSecret(): OpenAiSecretState {
  ...
  return {
    ...
    startsWith: value ? value.slice(0, 8) : "",
    endsWith: value ? value.slice(-4) : "",
...
export function getOpenAiSecretForAuth(): { value: string; diagnostics: OpenAiSecretDiagnostics } {
  ...
  if (!value) throw new Error("Provider 'openai' is not configured. Set OPENAI_API_KEY in backend secrets.");
  if (diagnostics.malformedReasons.length > 0) {
    throw new Error(`OPENAI_API_KEY is malformed: ${diagnostics.malformedReasons.join(" ")}`);
  }
...
export async function readOpenAiError(response: Response): Promise<OpenAiErrorInfo> {
  ...
  try {
    ...
    return { status: response.status, message: redactKeyLikeTokens(text.slice(0, 500) || `OpenAI HTTP ${response.status}`) };
  }
}