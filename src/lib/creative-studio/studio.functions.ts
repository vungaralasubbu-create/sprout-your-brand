...
async function loadBrandKit(supabase: any, brandKitId: string | null | undefined): Promise<BrandKitSnapshot | null> {
  ...
  const { data } = await ((supabase) as any).from("cs_brand_kits").select("*").eq("id", brandKitId).maybeSingle();
  if (!data) return null;
  return {
    id: data.id,
    name: data.name,
...
export const createBrandKit = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = id
      ? await ((supabase) as any).from("cs_brand_kits").update(payload).eq("id", id).eq("owner_id", userId).select("*").single()
      : await ((supabase) as any).from("cs_brand_kits").insert(payload).select("*").single();
