...
export async function loadKnowledgeSnapshot(): Promise<KnowledgeSnapshot> {
  ...
  const [coursesRes, catsRes, faqsRes, kbRes] = await Promise.all([
    supabaseAdmin
      .from("courses")
      .select("id,slug,title,category_slug,duration,price_range,mode,tags,career_outcomes")
      .eq("status", "published")
      .limit(80),
    supabaseAdmin.from("course_categories").select("slug,name,description").limit(40),
    supabaseAdmin.from("faqs").select("question,answer,category,priority").eq("published", true).limit(60),
    supabaseAdmin
  ...
  const courses = (coursesRes.data ?? []).map((c) => ({
    ...
    category: (c.category_slug as string | null) ?? null,
    duration: (c.duration as string | null) ?? null,
    priceLabel: (c.price_range as string | null) ?? null,
    mode: (c.mode as string | null) ?? null,
    tags: (c.tags as string[] | null) ?? [],
    outcomes: (c.career_outcomes as string | null) ?? null,
  }));
  ...
  const faqs = [
    ...(faqsRes.data ?? []).map((f) => ({
      ...
      topic: (f.category as string) ?? "general",
    ...
    ...(kbRes.data ?? []).map((k) => ({
      ...
      priority: ((k.priority as number) ?? 0) + 100, // curated wins
    })),
  ];
...
export function renderKnowledgePrompt(snap: KnowledgeSnapshot): string {
  ...
  lines.push("- Company: Glintr — EdTech platform helping sales professionals become entrepreneurs.");
  ...
  lines.push("- Programs live at /programs. Detail pages at /programs/{slug}.");
  lines.push("");
  ...
  lines.push("");
  lines.push("## LIVE PROGRAMS (current DB — cite exact titles and slugs)");
  ...
  lines.push("");
  if (snap.faqs.length) {
    lines.push("## CURATED Q&A (use answers here verbatim when the question matches)");
