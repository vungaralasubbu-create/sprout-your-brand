...
export const listAdminLeads = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [{ data: partners }, { data: courses }] = await Promise.all([
      partnerIds.length ? context.supabase.from("partners").select("id, full_name, email").in("id", partnerIds) : Promise.resolve({ data: [] as any[] }),
...
export const listPayouts = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: partners } = partnerIds.length
      ? await context.supabase.from("partners").select("id, full_name, email, status").in("id", partnerIds)
      : { data: [] as any[] };
    const pmap = new Map((partners ?? []).map((p: any) => [p.id, p]));
...
export const getPayoutDetail = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [{ data: partner }, { data: bank }, { data: items }, { data: history }] = await Promise.all([
      context.supabase.from("partners").select("id, full_name, email, status, kyc_status, sales_model_approval_status").eq("id", payout.partner_id).maybeSingle(),
...
export const listPartnersLiteFinance = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    await ensureAdmin(context);
    const { data } = await context.supabase.from("partners").select("id, full_name, email").order("full_name").limit(500);
    return data ?? [];
  });
