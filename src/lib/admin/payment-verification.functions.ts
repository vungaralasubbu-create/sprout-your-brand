...
export const adminGetPaymentSubmission = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await supabase
      .from("partner_payment_submissions")
      .select(
        "id, partner_id, lead_id, course_id, plan, amount, payment_date, payment_method, utr_reference, utr_normalized, payment_link_id, partner_notes, admin_notes, proof_bucket, proof_path, proof_mime, status, submitted_at, is_duplicate_flag, reviewed_by, reviewed_at, courses:course_id(name), partner_leads:lead_id(full_name, mobile, status), partners:partner_id(partner_code, display_name, first_name, email), payment_links:payment_link_id(url)",
      )
      .eq("id", data.id)
      .maybeSingle();
    if (error) throw new Error(error.message);
    ...
    return {
      ...
      partner_name: ((row.partners?.display_name || row.partners?.first_name) as string) ?? "—",
      partner_email: (row.partners?.email as string) ?? "",
