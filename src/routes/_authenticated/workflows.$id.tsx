...
import {
  ArrowLeft, Play, Save, Undo2, Redo2, GitBranch, Rocket, History, CheckCircle2,
  XCircle, Clock, MessageSquare, Users as UsersIcon,
} from "lucide-react";
...
function WorkflowBuilder() {
  ...
  const [tab, setTab] = useState<"builder" | "history" | "runs" | "comments">("builder");
  ...
  useEffect(() => {
    ...
    if (!found) { nav({ to: "/workflows" }); return; }
    setWf(found);
  }, [id, nav]);
  ...
  function updateWf(next: Workflow, pushUndo = true) {
    ...
    autosaveTimer.current = setTimeout(() => { saveWorkflow(next); setSaving(false); }, 500);
    setSaving(true);
  }
  function manualSave() { if (!wf) return; setSaving(true); saveWorkflow(wf); setTimeout(() => setSaving(false), 300); }
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)]">
      <header className="border-b border-border/60 bg-white px-4 py-2.5 flex items-center gap-3 flex-wrap">
        <Link to="/workflows" className="text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" /></Link>
        ...
        <div className="ml-auto flex items-center gap-1">
          ...
          <Button size="sm" variant="ghost" onClick={() => setTab("comments")} className={tab === "comments" ? "bg-neutral-100" : ""}><MessageSquare className="size-3.5 mr-1" />Comments</Button>
      ...
      {tab === "builder" && (
        <div className="flex-1 min-h-0 grid grid-cols-[240px_1fr_320px]">
          ...
          <div className="relative bg-[radial-gradient(circle_at_1px_1px,rgb(0_0_0_/_0.06)_1px,transparent_0)] [background-size:20px_20px] bg-neutral-50 overflow-hidden">
      ...
      {tab === "comments" && (
        <div className="flex-1 min-h-0 overflow-auto p-6 bg-neutral-50">
          <div className="max-w-2xl mx-auto rounded-lg border border-border/60 bg-white p-6 text-center">
            <UsersIcon className="mx-auto size-6 text-muted-foreground" />
            <div className="mt-2 text-sm font-medium">Collaboration</div>
            <p className="mt-1 text-xs text-muted-foreground">Invite teammates to review, comment, and approve nodes. Mentions and multi-level approvals sync with the platform notification center.</p>
          </div>
        </div>
      )}
    </div>
