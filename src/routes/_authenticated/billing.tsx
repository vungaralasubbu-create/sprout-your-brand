...
import { BillingShell, formatInr } from "@/components/billing/billing-shell";
...
function BillingLayout() {
  return (
    <BillingShell>
      <BillingOverviewIfIndex />
      <Outlet />
    </BillingShell>
  );
}

// Renders overview only when we're at /billing (index). Nested routes render their own content via <Outlet />.
function BillingOverviewIfIndex() {
  if (typeof window !== "undefined" && window.location.pathname !== "/billing") return null;
...
function BillingOverview() {
  ...
  const limits = sub?.bill_plans?.limits ?? { ai_credits: 500, storage_gb: 1, team_members: 1 };
